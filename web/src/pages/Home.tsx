...
export default function Home() {
  ...
  return (
    <div className="page-container">
      ...
      <div className="relative rounded-3xl overflow-hidden bg-gradient-to-br from-slate-50 via-slate-100/50 to-blue-50/30 p-6 sm:p-8 border border-slate-100">
        ...
        <div className="relative space-y-6">
          ...
          <div className="space-y-1">
            <p className="text-xs font-semibold text-brand-600 uppercase tracking-wider">{greeting()}, {user?.name?.split(' ')[0] || 'Explorer'} 👋</p>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-xl overflow-hidden">
            {/* Tab Bar */}
            <div className="flex border-b border-slate-100 px-4 pt-3 overflow-x-auto no-scrollbar">
              {[
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-4 py-2.5 text-xs font-semibold border-b-2 transition-all whitespace-nowrap -mb-px ${
                      isActive 
            ...
            <form onSubmit={handleSearch} className="p-4">
              {searchTab === 'explore' && (
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="flex-1 flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus-within:bg-white focus-within:ring-2 focus-within:ring-brand-500/20 focus-within:border-brand-500 transition-all">
                  ...
                  <button type="submit" className="btn btn-primary px-6 py-3 rounded-xl text-sm font-semibold whitespace-nowrap">
                    Search Places
                  </button>
                </div>
              )}
              ...
              {searchTab === 'flights' && (
                <div className="flex flex-col md:flex-row gap-3">
                  ...
                  <button type="submit" className="btn btn-primary px-6 py-3 rounded-xl text-sm font-semibold">
                    Find Flights
                  </button>
                </div>
              )}
              ...
              {searchTab === 'hotels' && (
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5">
                      <span className="text-xs font-semibold text-slate-500">Guests:</span>
                      <select
                        value={hotelGuests}
      ...
      <section>
        <div className="section-header">
          <h2 className="section-title">Book & Plan</h2>
        </div>
          ...
          {CATEGORIES.map(cat => {
            ...
            return (
              <button
                ...
                className="flex flex-col items-center gap-2.5 p-3 rounded-3xl bg-white border border-slate-100 shadow-sm hover:shadow-card-hover hover:-translate-y-1 transition-all duration-300 group"
              >
                <div className={`w-12 h-12 ${cat.color} rounded-2xl flex items-center justify-center group-hover:scale-105 transition-transform duration-300 shadow-sm`}>
      ...
      <section>
        <div className="section-header">
          ...
          <button onClick={() => navigate('/reels')} className="section-link flex items-center gap-1">
        ...
        <div className="flex gap-5 overflow-x-auto no-scrollbar">
          {STORIES.map(story => (
            ...
            >
              ...
              <span className="text-[11px] font-bold text-slate-600 group-hover:text-slate-900 transition-colors">{story.name}</span>
            </button>
          ))}
        </div>
      </section>
      ...
      <section>
        <div className="section-header">
          ...
          <button onClick={() => navigate('/social')} className="section-link flex items-center gap-1">
            ...
            >
              ...
              <div className="relative h-48 overflow-hidden">
                ...
                >
                  <Heart
                    size={14}
                    className={savedCards.includes(dest.name) ? 'fill-red-500 text-red-500' : 'text-slate-600'}
                  />
                </button>
                {/* Tag */}
                <span className="absolute top-3 left-3 badge badge-blue text-[10px]">
                  <TrendingUp size={9} /> {dest.tag}
                </span>
              </div>
              ...
              <div className="p-4">
                ...
                <p className="text-xs text-slate-400 mt-0.5 line-clamp-1">{dest.desc}</p>
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center gap-1">
                    <Star size={11} className="fill-amber-400 text-amber-400" />
                    <span className="text-xs font-semibold text-slate-700">{dest.rating}</span>
                    <span className="text-xs text-slate-300">• {dest.temp}</span>
                  </div>
                  <span className="text-sm font-bold text-brand-600">{dest.cost}<span className="text-xs font-normal text-slate-400">/pp</span></span>
                </div>
              </div>
            </div>
          ))}
      ...
      <section>
        <div className="section-header">
          ...
          <span className="badge badge-red text-[10px]">🔥 Limited Time</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {DEALS.map((deal, idx) => (
            <div
              ...
              className="card card-hover cursor-pointer p-4 flex items-center gap-4"
            >
      ...
      <section>
        <div
          ...
          className="relative overflow-hidden rounded-3xl bg-brand-600 p-7 cursor-pointer group hover:bg-brand-700 transition-colors"
        >
          <div className="relative z-10">
            <span className="badge bg-white/20 text-white text-[10px] mb-3 inline-flex">✨ AI Powered</span>
            <h2 className="text-xl font-bold text-white mb-1">Plan Your Perfect Trip</h2>
            <p className="text-brand-100 text-sm mb-5">Tell us your budget & preferences. Our AI builds a complete day-wise itinerary in seconds.</p>
